...
class WindowPreview extends Tooltips.TooltipBase {
  constructor(item, metaWindow, previewScale, showLabel) {
    ...
    this.labelBin.set_width(
      WINDOW_PREVIEW_WIDTH * this.thumbScale * this.uiScale
    );
  ...
  _onEnterEvent(actor, event) {
    if (this._applet._tooltipShowing) this.show();
    else if (!this._showTimer)
      this._showTimer = Mainloop.timeout_add(
        300,
  ...
  show() {
    if (!this.actor || this._applet._menuOpen) return;
  ...
  _set_position() {
    ...
    if (this._applet.orientation === St.Side.BOTTOM) {
      ...
      previewTop =
        this.item.get_transformed_position()[1] +
        this.item.get_transformed_size()[1] +
        5;
    } else {
    ...
    if (
      ...
      this._applet.orientation === St.Side.TOP
    ) {
      // centre the applet on the window list item if window list is on the top or bottom panel
      previewLeft =
        this.item.get_transformed_position()[0] +
        this.item.get_transformed_size()[0] / 2 -
        previewWidth / 2;
    } else if (this._applet.orientation === St.Side.LEFT) {
      previewLeft =
        this.item.get_transformed_position()[0] +
        this.item.get_transformed_size()[0] +
        5;
    } else {
    ...
    previewLeft = Math.min(
      previewLeft,
      monitor.x + monitor.width - previewWidth
    );

    previewTop = Math.round(previewTop);
    previewTop = Math.min(
      previewTop,
      monitor.y + monitor.height - previewHeight
    );
...
class AppMenuButton {
  constructor(applet, metaWindow, alert) {
    ...
    if (this.metaWindow.progress !== undefined) {
      ...
      } else this.progressOverlay.hide();
      this._updateProgressId = this.metaWindow.connect(
        "notify::progress",
        () => {
    ...
    global.settings.connect(
      ...
      Lang.bind(this, this.onPanelEditModeChanged)
    );
    ...
    if (this.alert) this.getAttention();

    this.window_signals.connect(
      this.metaWindow,
      "notify::title",
      this.setDisplayTitle,
      this
  ...
  onPreviewChanged() {
    ...
    if (this._applet.usePreview)
      this._tooltip = new WindowPreview(
        this,
        this.metaWindow,
        this._applet.previewScale,
        this._applet.showLabel
      );
    else
      this._tooltip = new Tooltips.PanelItemTooltip(
        this,
        "",
        this._applet.orientation
      );
  ...
  onPanelEditModeChanged() {
    ...
    if (this._draggable) this._draggable.inhibit = editMode;
    this.actor.reactive = !editMode;
  }

  onScrollModeChanged() {
    if (this._applet.scrollable) {
      this.scrollConnector = this.actor.connect(
        "scroll-event",
  ...
  _onScrollEvent(actor, event) {
    ...
    //                   v   home-made xor
    if ((direction == 0) != this._applet.reverseScroll) i++;
    else i--;

    if (i == windows.length) i = 0;
    else if (i == -1) i = windows.length - 1;
  ...
  setDisplayTitle() {
    ...
    if (this._tooltip && this._tooltip.set_text) this._tooltip.set_text(title);
  ...
  onFocus() {
    if (this._hasFocus()) {
      ...
      this.actor.remove_style_class_name(
        "window-list-item-demands-attention-top"
      );
      this._needsAttention = false;
  ...
  _onButtonRelease(actor, event) {
    this._tooltip.hide();
    if (this.alert) {
      if (event.get_button() == 1) this._toggleWindow(false);
      return false;
    }

    if (event.get_button() == 1) {
      if (this.rightClickMenu.isOpen) this.rightClickMenu.toggle();
  ...
  _getPreferredWidth(actor, forHeight, alloc) {
    ...
    if (
      ...
      if (this._applet.buttonsUseEntireSpace) {
        let [lminSize, lnaturalSize] = this._label.get_preferred_width(
          forHeight
        );
        let spacing = this.actor.get_theme_node().get_length("spacing");
        alloc.natural_size = Math.max(
          this._applet.buttonSize * global.ui_scale,
  ...
  _allocate(actor, box, flags) {
    ...
    if (this.labelVisible) {
      ...
      childBox.x1 =
        box.x1 + Math.floor(Math.max(0, allocWidth - naturalWidth) / 2);
      childBox.x2 = Math.min(childBox.x1 + naturalWidth, box.x2);
    }
  ...
  setIcon() {
    ...
    if (old_child) old_child.destroy();
  }
    ...
    if (counter < 4) {
      Mainloop.timeout_add(
        ...
        Lang.bind(this, function() {
          if (
            this.actor.has_style_class_name(
...
class AppMenuButtonRightClickMenu extends Applet.AppletPopupMenu {
  ...
  _populateMenu() {
      ...
      if (mw.is_on_all_workspaces()) {
        ...
        for (let i = 0; i < length; i++) {
          ...
          let name = Main.workspace_names[i]
            ? Main.workspace_names[i]
            : Main._makeDefaultWorkspaceName(i);
          let ws = new PopupMenu.PopupMenuItem(name);

          if (i == curr_index) ws.setSensitive(false);
    ...
    item.connect(
      "activate",
      Lang.bind(this._launcher._applet, this._launcher._applet.openAbout)
    );
    ...
    item.connect(
      "activate",
      Lang.bind(this._launcher._applet, this._launcher._applet.configureApplet)
    );
    subMenu.menu.addMenuItem(item);

    item = new PopupMenu.PopupIconMenuItem(
      _("Remove 'Window list'"),
      "edit-delete",
      St.IconType.SYMBOLIC
    );
    item.connect(
    ...
    item.connect(
      ...
      Lang.bind(this, function() {
        for (let window of this._windows)
          if (window.actor.visible && !window._needsAttention)
  ...
  _onToggled(actor, isOpening) {
    if (this.isOpen) this._launcher._applet._menuOpen = true;
    else this._launcher._applet._menuOpen = false;
  }
...
class CinnamonWindowListApplet extends Applet.Applet {
  constructor(orientation, panel_height, instance_id) {
    ...
    this.signals.connect(
      ...
      this._onWindowMonitorChanged,
      this
    );
    this.signals.connect(
      global.screen,
      "window-workspace-changed",
      this._onWindowWorkspaceChanged,
      this
    );
    this.signals.connect(
      global.screen,
      "window-skip-taskbar-changed",
      this._onWindowSkipTaskbarChanged,
      this
    );
    this.signals.connect(
      global.screen,
  ...
  on_orientation_changed(orientation) {
    ...
    for (let window of this._windows) window.updateLabelVisible();
    ...
    if (orientation == St.Side.TOP) {
      ...
      for (let child of this.manager_container.get_children()) {
        ...
        child.set_style(
          "margin-left 0px; padding-left: 0px; padding-right: 0px; margin-right: 0px;"
        );
  ...
  _onWindowAdded(screen, metaWindow, monitor) {
    if (this._shouldAdd(metaWindow)) this._addWindow(metaWindow, false);
  }
  ...
  _onEnableScrollChanged() {
    for (let window of this._windows) window.onScrollModeChanged();
  }

  _onPreviewChanged() {
    for (let window of this._windows) window.onPreviewChanged();
  }
  ...
  _onWindowDemandsAttention(display, window) {
    ...
    if (!this._windows[i].getAttention()) return;
  ...
  _refreshItem(window) {
    window.actor.visible =
      window.metaWindow.get_workspace() ==
     ...
     * one isn't shown! */
    if (window.alert) window.actor.visible = !window.actor.visible;

    if (window.actor.visible) window.setIcon();
  }
  ...
  _updateWatchedMonitors() {
    ...
    if (n_mons == 1) {
      ...
      for (let i = 0; i < n_mons; i++)
        if (instances.indexOf(i) == -1) this._monitorWatchList.push(i);
    }
    ...
    for (let window of windows) {
      if (this._shouldAdd(window)) this._addWindow(window, false);
      else this._removeWindow(window);
    }
  }

  _addWindow(metaWindow, alert) {
    for (let window of this._windows)
      if (window.metaWindow == metaWindow && window.alert == alert) return;
  ...
  handleDragOver(source, actor, x, y, time) {
    ...
    let isVertical =
      this.manager_container.height > this.manager_container.width;
    ...
    if (this._dragPlaceholder == undefined) {
      ...
      this.manager_container.insert_child_at_index(
        this._dragPlaceholder.actor,
  ...
  erodeTooltip() {
    ...
    this._tooltipErodeTimer = Mainloop.timeout_add(
      300,
      Lang.bind(this, function() {
        this._tooltipShowing = false;
        this._tooltipErodeTimer = null;
